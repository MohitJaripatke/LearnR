...
We are trying to analyze the large set of news articles to help make it easier for common people to filter through many articles related to the virus, and find their own resoluteness.
Furthermore, we would want to understand the semantic relations between different topics. And finally, analyze keywords to uncover patterns in the news content.
...
We set out to answer the following questions with our project 
	...
	*Classification and Prediction  of  news articles on Coronavirus(multilabel classification)
	*Comparative analysis of feature extraction methods like word2vec, Doc2vec, TD-IDF, IDF, BOW.
	*Analysing the semantic relatedness between different articles by Clustering.
	*Topic modelling through LDA to find prevalent keywords representing each topic.
	*Analyzing keywords to uncover patterns in news content.
	*Analyzing different classifiers performance over the dataset.
	*Visualization of insights after classification.
